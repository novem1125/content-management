...
export class AuthService {
  ...
  async userRegister(body: userRegister): Promise<userResponse> {
    ...
    const existing = await this.authRepository.findByEmailOrPhone(
      body.emailOrPhone,
    );
    if (existing) {
      throw new HttpError(
  ...
  async login(
    ...
    if (!existingSession) {
      newSession = await this.authRepository.createSession(
        sessionToken,
        user.id,
        userAgent,
        userIp,
      ...
      newSession = await this.authRepository.updateSession(
        sessionToken,
        user.id,
        userAgent,
        userIp,
    ...
    const payload: JwtPayload = {
      ...
      session_token: newSession.refreshToken, // session token from DB
      session_id: newSession.id,
    };

    // Secret key from env
    const secretKey =
      process.env.JWT_SECRET || process.env.SECRET_KEY || "contentusersecret";
  ...
  async googleLogin(idToken: string): Promise<any> {
    try {
      ...
      if (!session) {
        ...
        session = await this.authRepository.updateSession(
          sessionToken,
          user.id,
        );
      }
      ...
      const jwtPayload = {
        ...
        phone_no: user.phone_no ?? null,
        role_id: user.role_id ?? null,
        session_token: session?.refreshToken!,
