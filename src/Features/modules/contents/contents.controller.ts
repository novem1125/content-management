...
export class ContentController {
  ...
  getContentByUser = async (c: Context) => {
    try {
      const user = c.get("user");
      const item = await this.contentService.getByUserId(user.id);
      return c.json({ success: true, data: item }, 200);
    } catch (err: any) {
      console.error("getById Error:", err);
      if (err.message === "NOT_FOUND") {
  ...
  upload = async (c: Context) => {
    try {
      ...
      return c.json(
        {
          success: false,
          message: err.message || "Failed to upload file to MinIO",
        },
        500,
      );
    }
    ...
    try {
      ...
      if (!userId) {
        return c.json(
          {
            success: false,
            message: "Unauthorized: User ID missing from request context",
          },
          401,
        );
      }
      ...
      const body =
        c.get("validJson") ||
        (await c.req.parseBody({ all: true }).catch(() => ({})));

      const newContent = await this.contentService.createContent(userId, body);
